...
import { useAtmosphere } from './Atmospheres.tsx';
...
function IndexPage({ name, time }: { name: string, time: number }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- init once; guarded by mainParticleRef
  }, []);

  useEffect(() => {
    if (isModelLoaded && (mainParticleRef.current != null)) {
      mainParticleRef.current.ChangeModel(name, time);
    }
  }, [name, isModelLoaded, time]);
